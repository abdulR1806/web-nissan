...
# Read the HTML file
with open("lokasi.html", "r", encoding="utf-8") as f:
    html_content = f.read()
...
def replace_url_with_local(match):
    ...
    # Skip if not a readdy.ai URL or public.readdy.ai URL
    if "readdy.ai" not in url:
        return f"background-image: url('{url}')"
    
    # Handle map placeholder separately
    if "map_placeholder" in url:
        return f"background-image: url('readdy_images/map_placeholder_1280x720.png')"
...
with open("lokasi.html", "w", encoding="utf-8") as f:
    f.write(updated_html)

print("Updated all Readdy.ai image URLs in lokasi.html")
